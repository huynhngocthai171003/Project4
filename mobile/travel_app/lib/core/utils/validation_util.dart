...
class ValidateUtils {
  ...
  static String? validatePhoneNumberNotRequire(String value) {
    if (value.trim().isEmpty) {
      return null;
  ...
  static String? validateEmailNotRequire(String value) {
    if (value.trim().isEmpty) {
      return null;
    } else if (!RegExp(
            r"^[\w!#$%&'+\-/=?\^_`{|}~]+(\.[\w!#$%&'*+\-/=?\^_`{|}~]+)" +
  ...
  static String? vallidateEmailNullable(String value) {
    if (value.trim().isEmpty) {
      return null;
    } else if (!RegExp(r"^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$").hasMatch(value)) {
      return 'Email không hợp lệ';
