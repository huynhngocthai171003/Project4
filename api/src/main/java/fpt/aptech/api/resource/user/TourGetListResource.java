...
public class TourGetListResource {
    ...
    public Booking bookTour(@RequestBody Booking tour) {
        return bookingTourService.bookTour(tour);
    }

    @PostMapping("/payment")
    public void payment(@RequestBody Payment pay) {
        bookingTourService.payment(pay);
    }
    
    @GetMapping("/order/{userId}")
    public List<Booking> getTourByUserId(@PathVariable Integer userId) {
        return bookingTourService.getTourOrderById(userId).subList(0, 3);
    }
